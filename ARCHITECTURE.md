...
```mermaid
---
title: nom-nom — Architecture
---
graph TD
    subgraph Frontend
        NextJS[Next.js] -->|uses| TailwindCSS[Tailwind CSS]
        NextJS -->|uses| RadixUI[Radix UI]
        NextJS -->|uses| Shadcn[Shadcn]
        NextJS -->|uses| Sonner[Sonner]
        subgraph Client
            ClientNode[Client] -->|uses| React
            ClientNode -->|uses| ReactQuery
            ClientNode -->|uses| ReactHookForm
            ClientNode -->|uses| Zod
        end
    end
    subgraph Backend
        subgraph API
            TRPC -->|uses| Drizzle
            TRPC -->|uses| PostgreSQL
            TRPC -->|uses| Clerk
            TRPC -->|uses| Inngest
            subgraph Endpoints
                APINode[API] -->|endpoint| Webhooks
                APINode -->|endpoint| TRPCAPINode[TRPC API]
            end
        end
        subgraph Database
            Drizzle -->|manages| PostgreSQL
        end
    end
    subgraph ThirdPartyServices
        Clerk -->|handles| AuthenticationNode[Authentication]
        Inngest -->|handles| BackgroundJobsNode[Background Jobs]
    end
    NextJS -->|consumes| TRPC
    TRPC -->|interacts with| PostgreSQL
    TRPC -->|interacts with| Clerk
    TRPC -->|interacts with| Inngest
```
